...
func registerRunTool(server *mcp.Server, application *app.App, opts Options) error {
	tool := &mcp.Tool{
		Name:        "vers.run",
		Description: "Start a Vers environment (cluster) using inputs similar to vers run",
	}
	handler := withMetrics("vers.run", func(ctx context.Context, req *mcp.CallToolRequest, in RunInput) (*mcp.CallToolResult, presenters.RunView, error) {
		...
		summary := redact(fmt.Sprintf("cluster started: clusterID=%s rootVmID=%s head=%s", res.ClusterID, res.RootVmID, res.HeadTarget))
		fmt.Fprintf(os.Stderr, "[mcp] tool=vers.run ok dur=%s clusterID=%s\n", time.Since(started).Truncate(time.Millisecond), res.ClusterID)
