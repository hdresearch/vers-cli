...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
    ...
    case vmsLoadedMsg:
        m.loading = false
        if msg.err != nil { m.status = "Failed to load VMs: " + msg.err.Error(); return m, nil }
        m.vms.SetItems(msg.items)
        if len(msg.items) > 0 { m.vms.Select(0) }
        return m, nil

    case actionCompletedMsg:
        if msg.err != nil { m.status = msg.text + ": " + msg.err.Error() } else { m.status = msg.text }
        m.loading = true
        return m, refreshCurrentVMsCmd(m)
    case historyLoadedMsg:
        m.status = ""
        if msg.err != nil { m.status = "History error: " + msg.err.Error(); m.modalActive = false; return m, nil }
    ...
    case treeLoadedMsg:
        m.status = ""
        if msg.err != nil { m.status = "Tree error: " + msg.err.Error(); m.modalActive = false; return m, nil }
        m.modalText = msg.lines
        m.modalActive = true
        m.modalKind = "text"
        m.modalPrompt = "Tree:"
    ...
    case tea.KeyMsg:
        ...
        case "q", "ctrl+c":
            return m, tea.Quit
        case "tab":
            if m.focus == focusClusters { m.setFocus(focusVMs) } else { m.setFocus(focusClusters) }
            return m, nil
        }
        ...
        if m.focus == focusClusters {
            ...
            case "t":
                if cid, ok := m.selectedClusterID(); ok { m.status = "Loading tree..."; return m, loadTreeCmd(m, cid) }
            }
            return m, cmd
        }
        if m.focus == focusVMs {
            ...
            switch msg.String() {
            case "enter":
                if it, ok := m.vms.SelectedItem().(vmItem); ok { m.status = "Connecting..."; return m, doConnectCmd(m, it.ID) }
            case "b":
                if it, ok := m.vms.SelectedItem().(vmItem); ok {
                    ...
                    m.input.SetValue("")
                    id := it.ID
                    m.onSubmit = func(alias string) tea.Cmd { m.modalActive = false; m.status = "Branching..."; return doBranchCmd(m, id, alias) }
                    return m, nil
                }
            case "c":
                if it, ok := m.vms.SelectedItem().(vmItem); ok {
                    ...
                    m.input.SetValue("")
                    id := it.ID
                    m.onSubmit = func(tagCSV string) tea.Cmd { m.modalActive = false; m.status = "Committing..."; return doCommitCmd(m, id, tagCSV) }
                    return m, nil
                }
            case "p":
                if it, ok := m.vms.SelectedItem().(vmItem); ok {
            ...
            case "k":
                if it, ok := m.vms.SelectedItem().(vmItem); ok {
                    ...
                    id := it.ID
                    m.onConfirm = func() tea.Cmd { m.modalActive = false; m.status = "Deleting..."; return doKillVMCmd(m, id, m.recursiveVMKill) }
                    return m, nil
                }
            case "h":
                if it, ok := m.vms.SelectedItem().(vmItem); ok { m.modalActive = true; m.modalKind = "text"; m.modalPrompt = "History:"; m.status = "Loading history..."; return m, loadHistoryCmd(m, it.ID) }
            }
            var cmd tea.Cmd
    ...
    if m.modalActive {
        if m.modalKind == "input" {
            if km, ok := msg.(tea.KeyMsg); ok {
                if km.Type == tea.KeyEnter { if m.onSubmit != nil { return m, m.onSubmit(m.input.Value()) } }
                if km.Type == tea.KeyEsc { m.modalActive = false; return m, nil }
            }
            var cmd tea.Cmd
        ...
        if m.modalKind == "confirm" {
            if km, ok := msg.(tea.KeyMsg); ok {
                if (km.String() == "R" || km.String() == "r") && m.modalPrompt != "" {
                    ...
                    onoff := "off"; if m.recursiveVMKill { onoff = "on" }
                ...
                if km.String() == "y" || km.Type == tea.KeyEnter { if m.onConfirm != nil { return m, m.onConfirm() } }
                if km.String() == "n" || km.Type == tea.KeyEsc { m.modalActive = false; return m, nil }
            }
            return m, nil
        }
        if m.modalKind == "text" {
            if km, ok := msg.(tea.KeyMsg); ok {
                if km.Type == tea.KeyEsc || km.String() == "q" { m.modalActive = false; return m, nil }
            }
            return m, nil
        }
    }
