...
func RenderBranch(a *app.App, res BranchView) {
	...
	if res.UsedHEAD {
		fmt.Printf(s.Tip.Render("Using current HEAD VM: ") + s.VMID.Render(res.FromID) + "\n")
	}
	...
	fmt.Printf(s.ListItem.Render(s.InfoLabel.Render("VM ID")+": "+s.VMID.Render(res.NewID)) + "\n")
	if res.NewAlias != "" {
		fmt.Printf(s.ListItem.Render(s.InfoLabel.Render("Alias")+": "+s.BranchName.Render(res.NewAlias)) + "\n")
	}
	fmt.Printf(s.ListItem.Render(s.InfoLabel.Render("State")+": "+s.CurrentState.Render(res.NewState)) + "\n\n")
	...
	if res.CheckoutDone {
		...
		fmt.Printf(successStyle.Render("✓ HEAD now points to: ") + s.VMID.Render(display) + "\n")
		return
	}
